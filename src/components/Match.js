      ...
      {counter === 0 ? (
        <div className="flex mt-[50px] flex-nowrap justify-between items-center">
          <div className="flex flex-col mr-10">
            ...
            <div
              className={`bg-white bg-no-repeat bg-center w-[250px] h-[250px] rounded-[50%] border-[25px] border-${player} bg-${player} ${
                game[game.length - 1].result === 'player' ? 'shadow-' + player + '-active' : 'shadow-' + player
              } bg-50 `}
            ></div>
          </div>
          <div className="flex flex-col mx-8 text-center">
            <span className="uppercase text-5xl font-bold mb-[15px] text-white">
              {game[game.length - 1].result === 'player' ? 'You Win' : game[game.length - 1].result === 'computer' ? 'You Lose' : 'Draw'}
            </span>
            <Link to="/" className="bg-white uppercase py-[10px] px-[20px] rounded-[5px] text-[0.8rem]" onClick={handleClick}>
              {score.player - score.computer === 3 ? (
                <p>
                  You Win <br /> Again?
                </p>
              ) : score.computer - score.player === 3 ? (
                <p>
                  Computer Win <br /> Again?
                </p>
              ) : (
                'Play Again'
              )}
          ...
          <div className="flex flex-col ml-10">
            ...
            <div
              className={`bg-white bg-no-repeat bg-center w-[250px] h-[250px] rounded-[50%] border-[25px] border-${computer} bg-${computer} ${
                game[game.length - 1].result === 'computer' ? 'shadow-' + computer + '-active' : 'shadow-' + computer
              } bg-50 `}
            ></div>
          </div>
        </div>
